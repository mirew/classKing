...
public class MyPageQnaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int listCount = qService.myqListCount(memberId);
		ArrayList<Qna> list = qService.myqList(memberId, currentPage, limit);
		...
		for (Qna q : list) {
			...
			job.put("no", q.getQnaNo());
			job.put("qnatitle", q.getQnaTitle());
			job.put("answer",q.getQnaAnswer());
			job.put("date", q.getQnaDate().toString());
