...
<script type="text/javascript">
   window.onbeforeunload=exit;
    ...
    var webSocket = new WebSocket('ws://192.168.20.233:5500/classKing/broadcasting');
    ...
    function send() {
        if (inputMessage.value == "") {
            ...
            $("#messageWindow").html($("#messageWindow").html()
                + "<p style='text-align:right;'><strong><%=loginUser.getMemberId()%>"
                + "</strong></p><p style='text-align:right;'>" + inputMessage.value + "</p>");
            $("#td_1").scrollTop($("#td_1")[0].scrollHeight);
        }
        webSocket.send($("#chat_id").val() + "|" + inputMessage.value + "|" + <%=classNo%>);
        inputMessage.value = "";
    }
    ...
    function join(){
       webSocket.send("|" + $("#chat_id").val() + "|" + <%=classNo%>);
    }
    function exit(){
       webSocket.send("!|" + $("#chat_id").val() + "|" + <%=classNo%>);
    }
